...
3. **The model never sees long-lived credentials.** The control plane only ever
...
4. **The ChatGPT page DOM is untrusted territory.** The control-plane proxy
...
| Pairing code brute force | 8 chars from a 31-char CSPRNG alphabet (~40 bits), 5 attempts per session, per-IP rate limit (10/min; the IP key is the last X-Forwarded-For hop, which proxied clients cannot forge), 5-minute TTL, one-time use, session destroyed on limit |
...
| Sensitive files | Deny-by-default patterns (.env*, .envrc, keys, SSH, cloud creds, keychains, the whole `.git/` directory — remote credentials live in `.git/config`…) enforced at resolve time — reads, listings, and search all pass through the same gate; `git diff` adds pathspec excludes; `.env.example` allowed |
...
| Execution output leak | The agent may nominate test/build/lint logs; a local sanitizer redacts tokens, pairing-code-shaped strings and home paths, truncates size, and refuses private-key blocks entirely. Restricted items are listed without a body. ChatGPT still cannot run commands. |
...
| Unauthenticated endpoint flooding | Pending authorize pages capped (50); DCR client registrations capped (200, reset by `awehitch unpair`) and rate-limited per address for tunneled requests |
| Error detail leakage | Unhandled request errors return opaque JSON (no stack traces, no absolute paths); the details stay in the server log |
| Torn config writes | Every user-owned config/state write (codex `config.toml`, `opencode.json`, zcode `config.json`, runtime/auth state) is atomic (temp file + rename); the `writable_roots` upsert appends without rewriting existing entries |
| Scope escalation | A scope request grants the intersection with supported scopes only; an all-unknown request is rejected with `invalid_scope` instead of widening to full access |
...
(`~/Library/Application Support/awehitch` on macOS), directories 0700,
