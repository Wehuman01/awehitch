...
Do not re-pair or recreate the connector just to resume.
...
One ChatGPT conversation per task. `awehitch_open_chat` with a `task_id`
opens a fresh chat for a new task and reopens the bound chat when the same
task resumes (across review iterations and agent restarts). A replacement
chat is opened only when the bound chat is lost (404) or the user asks for
one (`fresh=true`).

Right after the boot prompt on a replacement chat, the agent calls
`awehitch_send_handoff`: the control-plane proxy composes the HANDOFF below
from the local session checkpoint — a brief, never a data dump (the new chat
re-reads code via MCP). The agent never assembles the brief by hand and
never pastes logs into it.

Trust order: connector (current code) > HANDOFF (this task).
