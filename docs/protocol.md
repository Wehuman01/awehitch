...
| HANDOFF | agent | Continuation brief sent to a replacement conversation |

There is no `STATE: RESUME`. If the agent restarts mid-task, it reads a **local
...
only the table above. If the original chat is gone, the agent sends HANDOFF
...
| `EXECUTING` | the agent is applying the current PLAN |
...
- **long-chat:** one long-lived C2C conversation per workspace. The agent opens a
  ...
  lost.
- **project:** one ChatGPT Project (collection) per workspace. A new agent
  conversation starts a new chat **inside that Project**. The same agent
  conversation keeps using its saved chat URL.

Right after the boot prompt, the agent sends a HANDOFF so the new chat can
...
`maxIterations` (default 12, configurable in `.c2c.json`). When reached, the agent
...
11. Be substantive. PLAN and review replies must carry enough signal for
    for the agent to act on: rationale, per-file natural-language suggestions
...
```
You are the planning and review layer for one local workspace. The agent executes.
