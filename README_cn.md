<div align="center">
  <h1>awemind：让 ChatGPT 网页版成为任意编码 agent 的外挂大脑</h1>
...
awemind 把 ChatGPT 网页版变成编码 agent 的"外挂大脑"：ChatGPT 负责规划与审查，本地 agent 负责执行。你的仓库永远不会被上传——ChatGPT 通过一条安全的、OAuth 保护的**只读** MCP 连接按需读取当前工作区里它真正需要的那几行代码。不用 API Key，不搞逆向代理。
...
awemind login -w /path/to/project        # 在打开的窗口里登录一次 ChatGPT
```
...
- **控制面** — agent 与 ChatGPT 交换极小的结构化 `[C2C]` 消息（`INIT → PLAN → EXECUTED → REVIEW → DONE`）。本地**控制面代理**用 Playwright（独立浏览器配置目录）把 ChatGPT 会话封装成四个语义化工具：`awemind_open_chat`、`awemind_send_state`、`awemind_wait_reply`、`awemind_read_reply`。轮询是 20–30 秒的廉价 DOM 检查；超时不等于失败；只用一个标签页；绝不因超时重发。原方案里绑死 Codex 内置浏览器的控制面被彻底解耦——任何 agent 只要"能调工具"就能接入。
...
awemind doctor -w <workspace> [--json]      # 诊断 + 自动修复
awemind login -w <workspace> [--json]       # 控制面 ChatGPT 登录
awemind pair | unpair -w <workspace>        # 配对码 / 吊销全部令牌
awemind session get|set|clear -w <workspace> # 会话 + 检查点
awemind sandbox-allow [--json]              # codex writable_roots（幂等）
```
...
corepack pnpm build     # -> dist/，暴露 awemind 命令
...
Alpha。控制面依赖当前 ChatGPT 页面结构；页面改版时 `awemind_wait_reply` 会诚实地报 `CHATGPT_DOM_CHANGED`——跑 doctor、修选择器。非 OpenAI 官方项目。
