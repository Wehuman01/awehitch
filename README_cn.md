<div align="center">
  <h1>awehitch：把 ChatGPT 网页大脑挂到你的工作区</h1>
  <p><strong>ChatGPT 负责思考——默认它还亲自动手。</strong></p>
  <p>用你已付费的 ChatGPT 网页订阅做编码大脑：直操模式（默认）下 ChatGPT 自己改文件、跑受限命令；接上任意编码 agent（codex / opencode / zcode）则退回"它规划审查、agent 完整保留执行权"的协作模式。</p>
  <p>
...
> ChatGPT 负责思考——默认它还亲自动手。

awehitch 把 ChatGPT 网页大脑挂（hitch）到你的机器上：**直操模式（默认）**下，ChatGPT 在自己的对话里用结构化补丁改文件、跑受限命令，不需要装任何本地 agent；**协作模式**下它规划与审查，本地 agent 执行。你的仓库永远不会被上传——ChatGPT 通过一条安全的、OAuth 保护的 MCP 连接按需读取当前工作区里它真正需要的那几行代码。不用 API Key，不搞逆向代理。
...
一个 agent 都不装？这就是默认体验：在你自己的 ChatGPT 对话里直接让它干活——它通过连接器读代码、打补丁、跑受限命令（直操模式，见下）。想要 agent 在环？正常使用即可："@chatgpt 帮我规划 XXX"。口语化的"用 ChatGPT …"、"问问 ChatGPT …"也可以——服务没跑时 agent 会按需自动拉起。

## 工作原理

```
远端大脑（ChatGPT 网页）
      ↕  控制面：[C2C] 状态消息（<1 KB）        —— 协作模式
控制面代理（本地，Playwright → 8 个语义化 MCP 工具）
      ↕  工具调用（stdio MCP）
本地 Agent（codex / opencode / zcode）
      ↕  数据面：直操 MCP（补丁 + 受限命令；       —— 两种模式共用
         配置 readonly 才退回只读）
...
- **数据面** — ChatGPT 通过 OAuth 2.1 + PKCE + 动态客户端注册的隧道自行拉取文件、diff、搜索结果、测试记录，外加直操写工具 `apply_patch` / `run_command`（默认就有；只有 `chatgptMode` 为 `readonly` 的工作区才从目录里消失）与 `dispatch_agent` 工具（独立 `dispatch.execute` 权限）在你点名时启动本地 agent。独立审查：EXECUTED 之后 ChatGPT 亲自看真实 git diff，绝不轻信"测试全过"。
...
### 直操模式（纯 ChatGPT）——默认，三档

开箱即用，ChatGPT **自己直接动手**：在自己的对话里读工作区、用结构化补丁改文件，默认档位还能跑受限命令，不需要装任何本地 agent。档位来自 `chatgptMode`（工作区 `.c2c.json`，或全局 `~/.c2c.json`），默认 `write-exec`。三档能力单调递增，写工具标注如实（`readOnlyHint: false`）：

| 档位 | ChatGPT 能做什么 |
| --- | --- |
| `readonly` | 只读工具 + `dispatch_agent`。结构性只读——服务端根本没有写工具。想只让它看、不让它动手就选这档。 |
| `write` | 加 `apply_patch`：结构化、原子、带回滚的多文件补丁（create / update / delete）。`update` 要求 `oldText` 与当前文件精确匹配且唯一，改错了就是整批拒绝。 |
| `write-exec`（默认） | 再加 `run_command`：argv 直传（无 shell，管道/展开/重定向结构性不存在）、最小环境变量（不继承任何秘密）、网络客户端与提权命令一律拒绝、git 的网络子命令（push/fetch/pull/clone）拒绝、默认 60 秒超时、输出封顶。 |

规则不变的部分：敏感文件策略（`.env*`、密钥、SSH…）对**写**一视同仁——直操模式也写不了它们；工作区边界与 symlink 检查照旧；没有 dangerous 档——想要无限制执行，那是本地沙箱工具的活，远程网页模型不背这个风险，这一档是**故意不存在**的。写授权由你写进配置文件（而不是 ChatGPT 的一句话），OAuth 侧对应 `workspace.write` / `exec.run` scope。

### 协作模式（接本地 agent）——谁发起都行，没有模式开关
...
```jsonc
{
  ...
  "chatgptMode": "write-exec"      // ChatGPT 直操档位：write-exec（默认）/ write / readonly。
                                   //   设 "readonly" 让连接器只看不动手；
                                   //   写授权永远写在你自己的配置里，不来自 ChatGPT 的一句话
}
```

`chatgptMode` 也可以写在全局 `~/.c2c.json`，对所有注册工作区生效；工作区自己的 `.c2c.json` 优先——目录没写这个键才落到全局，两边都没写就是 `write-exec`。改任一层都要重连一次（`awehitch up`）才生效。
...
- ChatGPT 拿到只读 scope（`workspace.read`、`workspace.search`、`git.read`、`execution.read`、`offline_access`）加一个 `dispatch.execute`——它只为你 @ 点名的请求启动 agent；默认直操模式另发 `workspace.write` / `exec.run` scope。访问令牌 1 小时失效，刷新令牌每次使用即轮换，落盘只存 SHA-256 哈希。真正的闸门始终是工作区的 `chatgptMode`：显式写了 `"readonly"` 的工作区，即使 scope 授了、写工具也连目录里都不会出现。令牌刷新时会自动补齐服务端新支持的 scope，所以以后升级不需要重新配对连接器。
...
- 协作模式的派发是硬约束：`dispatch_agent` 只有在 bridge 亲自读到你自己最新消息、且里面 @ 了执行者时才会启动（agent 注入的 [C2C] 消息永远不算）；读不出来就拒绝（fail closed）。没有你的 @，ChatGPT 只能用数据面工具干活。
- 任何档位下，仅有的写入口都是 `apply_patch`（结构化、原子、基线校验、敏感文件照拦）与 `run_command`（无 shell、最小环境、网络/提权/破坏性命令拒绝）——服务端不存在其他写入口，也没有无限制的 dangerous 档。想让 ChatGPT 只看不动手？写 `"chatgptMode": "readonly"`，写工具从目录里整个消失。`dispatch_agent` 只为用户 @ 点名的任务启动编码 agent，不接受 shell 命令。
